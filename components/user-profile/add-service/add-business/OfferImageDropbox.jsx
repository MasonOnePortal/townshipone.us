...
} from "@/store/Plan/PlanService";
import { useRemoveOfferImageMutation } from "@/store/offers/offerService";
import toast from "react-hot-toast";

export const OfferImageDropbox = ({
  imagesArrayList,
  updatedImageFiles,
  ...
  const [removeOfferImage] = useRemoveOfferImageMutation();
  ...
  // To validate the image resolution
  ...
  const deleteFiles = async (index, value) => {
    if (value && typeof value === "string" && !isBase64Image(value)) {
      ...
      if (offerId) {
        const { data: resData } = await removeOfferImage({
          id: offerId,
          data: data,
        });
        if (resData.ok) {
