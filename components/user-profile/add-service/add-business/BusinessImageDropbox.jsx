...
export const BusinessImageDropbox = ({
  imagesArrayList,
  updatedImageFiles,
  validHeight = 0,
  validWidth = 0,
  ...
  const [removeBusinessImage] = useRemoveBusinessImageMutation();
  ...
  // to validate the dimension of the image if applied
  ...
  const deleteFiles = async (index, value) => {
    if (value && typeof value === "string" && !isBase64Image(value)) {
      ...
      if (businessId) {
        const { data: resData } = await removeBusinessImage({
          id: businessId,
          data: data,
        });
        if (resData.ok) {
